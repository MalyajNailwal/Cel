...
* **Excel AI robustness gaps - research-driven priorities**: Web research on Microsoft's Excel Agent Mode reveals 3 critical gaps to close: (1) Formula Validation - verify syntax and cell references before applying; (2) Error Detection - pre-emptively catch #N/A, #DIV/0!, #REF! before they reach cells; (3) Operation Timeout - prevent infinite loops in multi-step plans. Priority 1 items to make Cel more robust than Microsoft Agent. Priority 2: enhance pre-execution review (human-in-loop confirming). Nice-to-have: session management, context compaction, audit log.
