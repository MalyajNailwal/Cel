...
* **Chart creation uses wrong columns - planner lacks header context**: AI planner doesn't receive column header info from selected range, causing it to generate wrong chart axes (e.g., 'Total Distance by Total Distance'). Root cause: selected\_range contains address but not column names; workbook\_context has sheet list but not headers. Fix: Extract column names from user message (e.g., 'Total Distance', 'Current Pressure'), match against selected range headers, inject into planner context. Similar pattern to border bug where params were incomplete. Frontend validation should parse chart columns from user message and fix params before execution.
