...
* **Critical failure detection + adaptive halt during execution**: Adaptive executor uses isCriticalFailure regex to detect errors that make subsequent steps impossible (sheet not found, range invalid, Excel not ready, worksheet does not exist). When detected, remaining steps are skipped and immediate reflection triggered for recovery. Non-critical errors (table overlap, formula errors, duplicate names) still cascade normally. Pattern: check error message against regex after each failed step, break execution loop if critical.

<!-- lore:019d774a-9880-7e96-af8a-e9e05b9e1d4a -->
* **Data Context Agent - deterministic profiling for planner**: Data profiling injected into planner context via deterministic profile\_data() function (backend/main.py). Computes null %, duplicate count, mixed type detection, outlier detection per column. Output injected via {data\_profile} placeholder in TASK\_DESCRIPTION format string. No LLM needed — pure computation on selected range data. Returns empty string if no data available. Gives planner richer context about actual data quality for smarter plan generation.
